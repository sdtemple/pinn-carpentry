...
if __name__ == "__main__": 
    ...
    axes[0].scatter(X_u_BC[:,0][::2]*factor, X_u_BC[:,1][::2]/4., marker='o', c='g', s=15, clip_on=False) # BC
    axes[0].scatter(x_obs_h[::2]*factor, t_obs_h[::2]/4., facecolors='none', edgecolors='k', marker='o', s=15, clip_on=False) # obs
    axes[0].scatter(X_h_IC[:,0]*factor, X_h_IC[:,1]/4., marker='*', c='r', s=25, clip_on=False) # snapshots
    ...
    cs = axes[2].contourf(xx, tt, h_pred2[:Nt_test,:]-h_test[:Nt_test,:], cmap='bwr', levels=levels, alpha = 0.8, extend='both')
    ...
    plt.tight_layout() 
    plt.savefig('figures/case3/contour.pdf')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('figures/case3/along_channel.pdf')
    plt.close()
