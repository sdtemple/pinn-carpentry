...
if __name__ == "__main__": 
    ...
    ax1.set_xticklabels([])
    ax1.text(0.05, 0.9, '{} PINN'.format(labels[1]), fontsize=16, transform=ax1.transAxes)

    ax2 = fig.add_subplot(gs[1, 2:])
    cs = ax2.contourf(xx, tt, h_interp[:Nt_test,:], cmap='rainbow', levels=levels, alpha = 0.8)
    ax2.set_xticklabels([])
    ax2.set_yticklabels([])
    ax2.text(0.05, 0.9, '{} Interpolation'.format(labels[2]), fontsize=16, transform=ax2.transAxes)
    ...
    ax3.text(0.05, 0.9, '{} PINN-Ref'.format(labels[3]), fontsize=16, transform=ax3.transAxes)

    ax4 = fig.add_subplot(gs[2, 2:])
    error2 = h_interp[:Nt_test,:]-h_test[:Nt_test,:]
    cs = ax4.contourf(xx, tt, error2, levels=levels_error, cmap='bwr', alpha = 0.8, extend="both")
    ax4.set_xlabel('Distance upstream (m)')
    ax4.set_yticklabels([])
    ax4.text(0.05, 0.9, '{} Interpolation-Ref'.format(labels[4]), fontsize=18, transform=ax4.transAxes)
    ...
    plt.subplots_adjust(left=0.1, 
                        ...
                        wspace=0.15, 
                        hspace=0.08)
    plt.savefig('figures/case5/contour.pdf')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('figures/case5/along_channel.pdf')
    plt.close()
