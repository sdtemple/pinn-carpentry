...
if __name__ == "__main__": 
    ...
    gs = gridspec.GridSpec(5, 8, hspace=0.08, wspace=0.1)
    ...
    ax0.scatter(x[0][0]-x_obs_h[::6]*factor, t_obs_h[::6], facecolors='none', edgecolors='k', marker='o', s=15, clip_on=False) # obs
    ax0.scatter(X_h_IC[:,0][::3]*factor, X_h_IC[:,1][::3], marker='*', c='r', s=25, clip_on=False) # snapshots
    ax0.set_ylabel('Time (h)')
    ax0.set_xticklabels([])
    ax0.text(0.05, 0.9, '{} Ref'.format(labels[0]), fontsize=fontsize_text, transform=ax0.transAxes)
    ...
    ax1.set_xticklabels([])
    ax1.text(0.05, 0.9, '{} PINN'.format(labels[1]), fontsize=fontsize_text, transform=ax1.transAxes)

    ax2 = fig.add_subplot(gs[1, 2:4])
    cs = ax2.contourf(xx, tt, h_pred_3obs[:Nt_test,:], cmap='rainbow', levels=levels, alpha = 0.8)
    ax2.set_xticklabels([])
    ax2.set_yticklabels([])
    ax2.text(0.05, 0.9, '{} PINN'.format(labels[2]), fontsize=fontsize_text, transform=ax2.transAxes) 

    ax3 = fig.add_subplot(gs[1, 4:6])
    cs = ax3.contourf(xx, tt, h_pred_4obs[:Nt_test,:], cmap='rainbow', levels=levels, alpha = 0.8)
    ax3.set_xticklabels([])
    ax3.set_yticklabels([])
    ax3.text(0.05, 0.9, '{} PINN'.format(labels[3]), fontsize=fontsize_text, transform=ax3.transAxes)

    ax4 = fig.add_subplot(gs[1, 6:])
    cs = ax4.contourf(xx, tt, h_pred_5obs[:Nt_test,:], cmap='rainbow', levels=levels, alpha = 0.8)
    ax4.set_xticklabels([])
    ax4.set_yticklabels([])
    ax4.text(0.05, 0.9, '{} PINN'.format(labels[4]), fontsize=fontsize_text, transform=ax4.transAxes) 
    ...
    cs = ax5.contourf(xx, tt, h_pred_2obs[:Nt_test,:]-h_test[:Nt_test,:], cmap='bwr', levels=levels_error, alpha = 0.8, extend='both')
    ax5.set_ylabel('Time (h)')
    ax5.set_xticklabels([])
    ax5.text(0.05, 0.9, '{} PINN-Ref'.format(labels[5]), fontsize=fontsize_text, transform=ax5.transAxes)

    ax6 = fig.add_subplot(gs[2, 2:4])
    cs = ax6.contourf(xx, tt, h_pred_3obs[:Nt_test,:]-h_test[:Nt_test,:], cmap='bwr', levels=levels_error, alpha = 0.8, extend='both')
    ax6.set_xticklabels([])
    ax6.set_yticklabels([])
    ax6.text(0.05, 0.9, '{} PINN-Ref'.format(labels[6]), fontsize=fontsize_text, transform=ax6.transAxes)

    ax7 = fig.add_subplot(gs[2, 4:6])
    cs = ax7.contourf(xx, tt, h_pred_4obs[:Nt_test,:]-h_test[:Nt_test,:], cmap='bwr', levels=levels_error, alpha = 0.8, extend='both')
    ax7.set_xticklabels([])
    ax7.set_yticklabels([])
    ax7.text(0.05, 0.9, '{} PINN-Ref'.format(labels[7]), fontsize=fontsize_text, transform=ax7.transAxes)

    ax8 = fig.add_subplot(gs[2, 6:])
    cs = ax8.contourf(xx, tt, h_pred_5obs[:Nt_test,:]-h_test[:Nt_test,:], cmap='bwr', levels=levels_error, alpha = 0.8, extend='both')
    ax8.set_xticklabels([])
    ax8.set_yticklabels([])
    ax8.text(0.05, 0.9, '{} PINN-Ref'.format(labels[8]), fontsize=fontsize_text, transform=ax8.transAxes)
    ...
    cs = ax9.contourf(xx, tt, h_interp_2obs[:Nt_test,:], cmap='rainbow', levels=levels, alpha = 0.8)
    ax9.set_ylabel('Time (h)')
    ax9.set_xticklabels([])
    ax9.text(0.05, 0.9, '{} Interpolation'.format(labels[9]), fontsize=fontsize_text, transform=ax9.transAxes)

    ax10 = fig.add_subplot(gs[3, 2:4])
    cs = ax10.contourf(xx, tt, h_interp_3obs[:Nt_test,:], cmap='rainbow', levels=levels, alpha = 0.8)
    ax10.set_xticklabels([])
    ax10.set_yticklabels([])
    ax10.text(0.05, 0.9, '{} Interpolation'.format(labels[10]), fontsize=fontsize_text, transform=ax10.transAxes)

    ax11 = fig.add_subplot(gs[3, 4:6])
    cs = ax11.contourf(xx, tt, h_interp_4obs[:Nt_test,:], cmap='rainbow', levels=levels, alpha = 0.8)
    ax11.set_xticklabels([])
    ax11.set_yticklabels([]) 
    ax11.text(0.05, 0.9, '{} Interpolation'.format(labels[11]), fontsize=fontsize_text, transform=ax11.transAxes)

    ax12 = fig.add_subplot(gs[3, 6:])
    cs = ax12.contourf(xx, tt, h_interp_5obs[:Nt_test,:], cmap='rainbow', levels=levels, alpha = 0.8)
    ax12.set_xticklabels([])
    ax12.set_yticklabels([])
    ax12.text(0.05, 0.9, '{} Interpolation'.format(labels[12]), fontsize=fontsize_text, transform=ax12.transAxes)
    ...
    ## Liear interpolation Error
    ax13 = fig.add_subplot(gs[4, :2])
    cs = ax13.contourf(xx, tt, h_interp_2obs[:Nt_test,:]-h_test[:Nt_test,:], cmap='bwr', levels=levels_error, alpha = 0.8, extend='both')
    ax13.set_xlabel('Distance upstream (m)')
    ax13.set_ylabel('Time (h)')
    ax13.text(0.05, 0.9, '{} Interpolation-Ref'.format(labels[13]), fontsize=fontsize_text, transform=ax13.transAxes)

    ax14 = fig.add_subplot(gs[4, 2:4])
    cs = ax14.contourf(xx, tt, h_interp_3obs[:Nt_test,:]-h_test[:Nt_test,:], cmap='bwr', levels=levels_error, alpha = 0.8, extend='both')
    ax14.set_xlabel('Distance upstream (m)')
    ax14.set_yticklabels([])
    ax14.text(0.05, 0.9, '{} Interpolation-Ref'.format(labels[14]), fontsize=fontsize_text, transform=ax14.transAxes)

    ax15 = fig.add_subplot(gs[4, 4:6])
    cs = ax15.contourf(xx, tt, h_interp_4obs[:Nt_test,:]-h_test[:Nt_test,:], cmap='bwr', levels=levels_error, alpha = 0.8, extend='both')
    ax15.set_xlabel('Distance upstream (m)')
    ax15.set_yticklabels([])
    ax15.text(0.05, 0.9, '{} Interpolation-Ref'.format(labels[15]), fontsize=fontsize_text, transform=ax15.transAxes)

    ax16 = fig.add_subplot(gs[4, 6:])
    cs = ax16.contourf(xx, tt, h_interp_5obs[:Nt_test,:]-h_test[:Nt_test,:], cmap='bwr', levels=levels_error, alpha = 0.8, extend='both')
    ax16.set_xlabel('Distance upstream (m)')
    ax16.set_yticklabels([])
    ax16.text(0.05, 0.9, '{} Interpolation-Ref'.format(labels[16]), fontsize=fontsize_text, transform=ax16.transAxes)
    ...
    plt.savefig('figures/case6/contour.pdf')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('figures/case6/along_channel.pdf')
    plt.close()
